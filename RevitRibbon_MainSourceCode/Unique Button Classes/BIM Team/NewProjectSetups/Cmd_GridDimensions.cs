...
namespace RevitRibbon_MainSourceCode
{
    ...
    public class Cmd_GridDimensions : IExternalCommand // Implementing IExternalCommand gets Revit to recognize our plugins
    {
        //public int dimsPlaced { get; set; }
        public static int dimsPlaced { get; set; } // This property is used to count the number of dimensions placed
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                using (Transaction t = new Transaction(doc))
                {
                    ...
                    {
                        //MyUtils.M_MyTaskDialog("Action Required", "No Dimention Type Template selected", "Warning");
            ...
            catch (Exception) // Catch errors
            {
                ...
                //exceptStatus.Show();
                MyUtils.M_MyTaskDialog("Selection Required", "There are no Grid Lines.", "Warning");
        ...
        public static void ShowResults(Document doc, double? offSet)
        {
            double? roundedNumber = offSet.HasValue ? Math.Round(offSet.Value, 1) : (double?)null;

            string messageinfo = roundedNumber.HasValue ? $"Current View Scale: {MyUtils.GetViewScaleString(doc.ActiveView)}" +
                                                          $"\nOffset Distance (ft): {offSet}" +
                                                          $"\nDimensions Placed: {dimsPlaced}"
                                                        : "The dimensions offset value is not available.";

            MyUtils.M_MyTaskDialog("Grid Dimensions", "Grid Dimensions Summary:", true, messageinfo);
        ...
        public bool SetRequiredGridsDimentionType(Document doc)
        {
            ...
            const string dimensionTypeTemplateName = "ACCO_Linear";

            RequiredGridsDimensionType = GetDimensionTypeByName(doc, gridDimensionTypeName);
        ...
        public DimensionType CreateGridDimensionType(Document doc, string dimensionTypeName)
        {
            ...
            if (newGridDimensionType == null)
            {
                ...
                // Duplicate the existing dimension type
                DimensionType newDimensionType = existingDimensionType.Duplicate("Temporary Name") as DimensionType;

                if (newDimensionType != null)
                {
                    // Check if the new name has been unexpectedly altered by the API
                    if (newDimensionType.Name != gridDimensionTypeName)
                    {
                        // Explicitly rename the dimension type to 'GRID DIMENSIONS'
                        try
                        {
                            newDimensionType.Name = gridDimensionTypeName;
                            if (newDimensionType.Name != gridDimensionTypeName)
                            {
                                // Handle the case where renaming was unsuccessful
                                // Log or throw an exception according to your needs
                                throw new Exception("The selected Dimension Type can not be renamed.");
                            }
                        }
                        catch (Exception ex)
                        {
                            // Handle any exceptions thrown during renaming
                            // Log or handle according to your needs
                            TaskDialog.Show("Error", $"Failed to rename dimension type: {ex.Message}");
                            return null;
                        }
                    }

                    // Return the new 'GRID DIMENSIONS' type
        ...
        public DimensionType CreateGridDimensionType2(Document doc, string dimensionTypeName)
        {
            string gridDimensionTypeName = "GRID DIMENSIONS";
            ...
            if (newGridDimensionType == null)
            {
                ...
                if (newDimensionType != null)
                {
                    //// Rename the new dimension type explicitly to 'GRID DIMENSIONS'
                    //newDimensionType.Name = gridDimensionTypeName;
        ...
        public List<DimensionType> GetExistingDimensionTypeList(Document doc)
        {
            ...
            var allDimensionTypes = dimCollector
                .OfClass(typeof(DimensionType))
                .Cast<DimensionType>()
                .Where(dt => dt.StyleType == DimensionStyleType.Linear) // Filter for linear dimension types
                .ToList();

            var renamableDimensionTypes = new List<DimensionType>();

            string tempName = "GRID DIMENSIONS";
            foreach (var dimensionType in allDimensionTypes)
            {
                // Temporarily duplicate the dimension type
                DimensionType tempDuplicate = dimensionType.Duplicate(tempName) as DimensionType;
                var dim1Id = dimensionType.Id;
                var dim2Id = tempDuplicate.Id;
                if (tempDuplicate != null)
                {
                    // Check if the duplicate's name is as expected
                    if (tempDuplicate.Name == tempName)
                    {
                        // If the name is as expected, this type is renamable
                        renamableDimensionTypes.Add(dimensionType);
                    }

                    // Delete the temporary duplicate
                    doc.Delete(tempDuplicate.Id);

                }
            }

            return renamableDimensionTypes;
        }


        public List<DimensionType> GetExistingDimensionTypeList2(Document doc)
        {
            var dimCollector = new FilteredElementCollector(doc);
        ...
        List<List<Dimension>> CreateDimensions(Document doc, List<Element> gridsCollector, XYZ vertPoint, XYZ horizPoint, int horizontalFeetOffSet, int verticalFeetOffSet, View curView)
        {
            ...
            Line line = Line.CreateBound(p1.Subtract(offsetVert), p2.Subtract(offsetVert));
            Dimension dim = doc.Create.NewDimension(curView, line, referenceArrayVertical, RequiredGridsDimensionType);
            if (dim != null)
                vertDimensionsList.Add(dim);

            // Create horizontal dimension line
            Line lineHoriz = Line.CreateBound(p1h.Subtract(offsetHoriz), p2h.Subtract(offsetHoriz));
            Dimension dimHoriz = doc.Create.NewDimension(curView, lineHoriz, referenceArrayHorizontal, RequiredGridsDimensionType);
            if (dimHoriz != null)
                horizDimensionsList.Add(dimHoriz);
